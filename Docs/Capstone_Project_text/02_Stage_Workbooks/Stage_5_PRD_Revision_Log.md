...
|---|---|
| What did you most misunderstand about the problem when you wrote version one? | That a 3-second reply was a software target. Two or three provider round trips make it a provider target; the local pipeline is 0.5 s. |
| Which piece of discovery work would have caught it earlier? | Latency: timing one live provider call during the setup step. |
| What would you do differently if you started this project again on Monday? | Run the end-to-end harness on 20 real tickets on day one, not after B-10 (the first live run exposed three false blocks, the memorised intent figure and the missing reconcile call). |
| What is still uncertain, and what would you need to resolve it? | False-block rate of 2 to 3%: needs agent review of blocked drafts in a pilot. |
