...
| Field | Value |
|---|---|
| Version | 2.0 (v1.0 dated 07/09/26; revised after the build and the validation run; every change and its trigger is in the Stage 5 revision log) |
| Written by | Anshul Ghogre |
| Date | 17/09/26 |
| Status | Revised after build |
| Approved by | Self |
...
| FR-02 | The system shall assign an intent (one of the 22 classes) and an urgency to every ticket with a calibrated confidence between 0 and 1, and record the alternatives considered. The intent is the weighted vote of the 7 nearest labelled development tickets (same embedder as retrieval); the language model supplies the instruction-like flag and the reason, and its intent is recorded as the first alternative when it disagrees. When the model fails, the reason reads "classification failed: provider unavailable" and the neighbours' intent is kept, so outages are not counted as unclear_request. | must | Section 1 Marcus row (no breakdown known); Section 3 step 2; Section 2 rows 3, 4 | Every processed ticket has intent, urgency, confidence in [0,1] and at least one alternative; a fallback class is returned when classification fails. |
...
| FR-04 | The system shall apply a relevance threshold (0.40 cosine, derived from development data) that removes off-topic passages and returns nothing rather than irrelevant text. The threshold does not detect non-answerable tickets; that is the router's job (never-auto intents and the confidence threshold, FR-05 and FR-06). | Must | Section 2 row 8 (143 non-answerable); Section 1 Ines row (novel incidents) | For feature_request and unclear_request tickets, retrieval returns nothing or the ticket is escalated; never an invented citation. |
...
| FR-07 | The system shall draft an answer only from the retrieved passages and say plainly when it does not know. Every factual sentence of the draft is verified against a retrieved passage (sentence cosine >= 0.50 or lexical containment >= 0.50, and NLI contradiction < 0.85); supported sentences carry the citation, attached by code where the model omitted it; an unverifiable sentence blocks the draft (FR-10). | Must | Section 1 Sofia row (draft plus page); Section 3 step 5; Section 5 risk 1 | Every citation resolves to a passage actually retrieved for that ticket; drafts with no retrieved passage contain no factual claims. |
...
| FR-10 | The system shall block any draft that contains a factual sentence not supported by the retrieved passages or contradicted by them (NLI check), or that states a refund has been issued, the issue is fixed on CloudServe's side, or a delivery date. Headings, symptom and cause lines of the articles, and courtesy or narration sentences are not claims. Blocked drafts travel to the engineer inside the escalation package; an expected false-block rate of 2 to 3% of tickets is accepted. | Must | Section 5 risks 1 and 6; Section 5 source 3 (the three universal forbidden claims) | A draft containing any forbidden phrase is blocked; a sample of 50 drafts reviewed shows hallucination at or below 5%. |
| FR-11 | The system shall write every automated decision (classification, routing, generation, validation) to a persistent log with input summary, prediction, confidence, alternatives, sources, threshold, action, reason, guardrail results, prompt version and requirement ids. | Must | Section 1 Marcus row (compliance review); Section 4 row 7 | Logged decisions reconcile exactly with tickets processed, including failures: the harness reconciles after every run and writes the result into metrics.json and run_summary.md, and writes routing and validation rows itself when a component raises (500/500 development, 80/80 validation). |
...
| FR-16 | The system shall flag every ticket with a predicted urgency and offer ordering by it in the batch output (--sort-urgency). The flag is advisory: predicted urgency is near the majority baseline (0.49 against 0.45 on development data) and must not be used for SLA decisions. | Could | Section 2 row 4; Section 1 Ravi row; Section 1 nobody-said row 3 | Output includes an urgency flag per ticket; ordering by urgency is available in the batch output. |
...
| FR-18 | The system shall expose a single-ticket API endpoint (POST /tickets) that accepts one ticket in the dataset schema and returns the decision, the draft with citations or the escalation package, and the guardrail verdicts; with GET /decisions/{ticket_id} for the log rows and GET /health for the kill-switch state. | Must | Build Specification section 06 steps 5 to 7 (graders submit one ticket per channel and a guardrail trigger by hand); Stage 5 revision log | One ticket per channel and a guardrail-trigger ticket submitted by hand return a decision without error; a malformed body returns an escalation, not a crash. Verified in the clean-clone rehearsal, 16/09/26. |
...
|---|---|---|---|
| NFR-01 | Latency | 95th percentile time per ticket under 3 s for the local pipeline (measured 0.53 s with cached provider replies). Provider round trips are reported separately (validation p95 13.7 s; live development run p95 16.8 s); the 3 s end-to-end figure is recorded as not achievable with a remote free-tier model making 2 to 3 calls per ticket. | Harness timing on the validation run, reported as median and p95, split into local pipeline and provider time. |
...
| NFR-06 | Fairness | Under 5 percentage points variation in resolution quality across fluency, tier, region, ticket length and channel, judged with 95% Wilson intervals; a point gap whose intervals overlap the best segment is reported, not treated as a finding. | python -m evaluation.fairness_audit over the validation and development runs; table in Docs/governance.md section 3. |
| NFR-07 | Cost | The paid llama-3.1-8b endpoint on OpenRouter at about $0.03 per 500 tickets (the free variant is capped at 50 requests a day and cannot complete a run); replies cached on disk; 20 requests per minute pacing with backoff; project total under $0.25. | Provider spend shown on the key's usage page after each run; cache hit rate visible in run duration. |
| NFR-08 | Calibration | Stated confidence within 5 points of observed accuracy in every band with at least 20 tickets; smaller bands are reported but not judged. | Calibration table in metrics.json (governance.calibration) on every run; ECE 0.005 development, 0.013 validation. |
...
|---|---|---|---|
| where does the 8 to 12 h first-reply figure come from, and can timestamps be supplied? | the SLA metric cannot be baselined. | Anshul Ghogre, to ask Marcus Adeyemi | Before a pilot; open at submission (20/09/26). Resolution time is the proxy until then. |
| Should billing and data-residency tickets auto-respond (labels) or always escalate (Daniel)? | scope of FR-06. | Anshul Ghogre, to ask Daniel Okonkwo | Before a pilot; open at submission. Labels' policy kept; tone guardrail covers the commitment risk. |
| What are the enterprise agreement terms? | NFR-06 and Marcus's warning. | Anshul Ghogre, to ask Marcus Adeyemi | Before a pilot; open at submission. Fairness audit reports tier segments meanwhile. |
| Who operates the kill switch, and how fast must it act? | governance framework requires one | Anshul Ghogre | Resolved 16/09/26: flag file or environment setting, any operator with shell access, effective on the next ticket (Docs/governance.md section 5). |
| Who scores the CSAT proxy rubric, and on what sample size? | target 4.0 needs a stated method. | Anshul Ghogre | Open at submission: PR-07 rubric exists; no scorer or sample size agreed. |
| Does CloudServe want replies labelled as automated? | Ravi wants it; Marcus did no say. | Anshul Ghogre, to ask Marcus Adeyemi | Built as FR-12 (disclosure on every automated reply); confirm with the client before a pilot. |
| Will Ines consolidate the private files into the knowledge base? | coverage could grow beyond 71.4%. |  | Out of scope |
| Does the system need a single-ticket API endpoint in addition to the batch harness? | Build Spec §06 says graders submit one ticket per channel and one guardrail-trigger ticket by hand before the batch run | Anshul Ghogre | Resolved 16/09/26: FR-18 built and rehearsed. |
